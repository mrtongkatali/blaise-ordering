...
v-layout(row, wrap)
  ...
  v-dialog(v-model="products.show", max-width="500px", persistent)
    v-form(ref="createProductForm")
      v-card
        v-card-title.title {{ (products.selected ? 'Update Product' : 'Add Product') }}
        v-card-text
          v-layout(row, wrap)
            v-flex(xs12)
              v-text-field(
...
export default {
  ...
  methods: {
    ...
    selectSupplier (supplier) {
      ...
      this.products.selected = supplier.id
